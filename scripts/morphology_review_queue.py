...
def main():
    ...
    print("lemma\tform\ttokens\treview_state\tanalysis_scope\tmatch_type\tsent_ids")
    for r in rows[:a.limit]:
        print(f'{r["lemma"]}\t{r["form"]}\t{r["tokens"]}\t{r["review_state"]}\t{r.get("analysis_scope") or "-"}\t{r.get("match_type") or "-"}\t{",".join(r["sent_ids"][:5])}')
