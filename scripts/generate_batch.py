...
from bororo_generator.batch import generate_batch,generate_batch_against_corpus,batch_summary
...
def main():
    ...
    p.add_argument("--corpus",help="Optional CorBo CoNLL-U file for exact-surface attestation")
    p.add_argument("--output")
    a=p.parse_args()
    req=json.loads(Path(a.requests).read_text(encoding="utf-8"))
    records=(generate_batch_against_corpus(req,a.corpus) if a.corpus
             else generate_batch(req))
