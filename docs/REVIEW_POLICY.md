...
A full reviewed stem class does not automatically classify every corpus form
bearing that lemma as reviewed morphology. A corpus FORM is marked reviewed only
when it exactly matches a predicate surface generated by an already licensed
request. This prevents unknown derivation, inflection, annotation error, or
constructional material from being hidden by a lexical class assignment.


The CLI also provides `--reviewed-only` as an audit view of corpus forms that
already match licensed generated predicate surfaces. This is complementary to
`--needs-only` and does not change review state or generation licensing.
