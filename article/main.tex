...
The study evaluates this architecture in three stages. First, a frozen controlled
...
compare two uses of the same LLM over a common frozen task inventory. These two
stages establish functional correctness inside a reviewed domain and are not
treated as estimates of performance on unseen linguistic material. Third, we
construct a source-group holdout from independently partitioned annotated
CorBo material, build grammar-v2 from the development partition only, freeze
its lexical licenses, and then open the held-out sources for evaluation. In the
...
The contribution is methodological. The architecture makes
...
reviewed in an imperative, subjunctive, gerundial, optative, negative, or irrealis
...
a different computational question. NüshuRescue uses an LLM primarily to expand parallel data
...
The baseline consequently provides evidence of controlled recombination under the
...
\section{Experiment 3: source-group holdout evaluation}
\label{sec:holdout}

The first two experiments deliberately operate inside a domain represented in
the frozen grammar. They therefore establish functional correctness of the
pipeline rather than performance on unseen linguistic material. To test
generalization separately, we constructed a second evaluation in which the
corpus was partitioned by documentary source group before grammar-v2 was
reviewed.

After excluding four singleton records whose provenance required separate
adjudication, the eligible annotated corpus contained 784 sentences. Whole
source groups, rather than individual sentences, were assigned to development
or test. The development partition contains 627 sentences (79.97\%); the
held-out partition contains 157 sentences (20.03\%) from ABE, BEBE, and C.O.
No test token or annotation was used to add or revise a grammar-v2 license
before the grammar was frozen. Three ABE examples and two BEBE examples had
been accidentally exposed during earlier corpus inspection; consequently the
holdout is reported as partially unblinded rather than as a pristine blind
test. None of those exposed examples was used to license a grammar-v2 rule.

Human review of the development partition yielded 28 accepted lexical licenses.
Three analyses (\textit{jorudu}, \textit{joruduwa}, and \textit{ciri}) remained
uncertain and were excluded. The accepted grammar was frozen before systematic
test evaluation. Corpus evidence alone did not create a license.

The held-out 157 sentences contain 194 tokens annotated as VERB or AUX. Of
these, 64 (33.0\%) have a lexical entry in the frozen grammar-v2 and 130
(67.0\%) fall outside its lexical coverage. Coverage is therefore reported
separately from compatibility: an unlicensed predicate is not counted as an
error.

A first mechanical comparison produced six apparent frame mismatches. All six
were causative forms whose base lemmas were correctly licensed as monovalent;
the evaluator had incorrectly applied the base valency directly to predicates
annotated \texttt{Voice=Cau}. The evaluation procedure was corrected
generically to treat causativization of a monovalent base as valency-increasing,
without changing any frozen lexical license. Re-running the same test produced
41 mechanical matches, 10 cases requiring object/annotation adjudication, and
13 cases requiring manual frame adjudication, with no remaining mechanical
frame mismatch.

Human adjudication classified two of the ten object cases as compatible with
the frozen analysis: one involved contextual object omission with
\textit{bito}, and one an implicit theme with \textit{maku}. The remaining
eight tokens all instantiated \textit{ro}. Post-test linguistic review
established that the frozen license \textit{ro}: divalent was incorrect.
Accordingly, these are reported as eight token-level conflicts but one
underlying lexical-license error. The frozen grammar was not retrospectively
changed.

Of the 13 manually assessed frame cases, 11 were compatible with the frozen
licenses. Two remained unresolved: a \textit{mako} occurrence with a
\textit{to} phrase and a causative \textit{aiwodo} construction. They are
retained as unresolved rather than forced into either success or error.

\begin{table}[ht]
\centering
\caption{Grammar-v2 source-group holdout evaluation.}
\label{tab:v2holdout}
\begin{tabular}{lr}
\toprule
Measure & Result\\
\midrule
Development sentences & 627\\
Held-out sentences & 157\\
Held-out predicate tokens & 194\\
Tokens with frozen lexical license & 64/194 (33.0\%)\\
Tokens outside lexical coverage & 130/194 (67.0\%)\\
Positively compatible covered tokens & 54/64 (84.4\%)\\
Conflicting covered tokens & 8/64 (12.5\%)\\
Unresolved covered tokens & 2/64 (3.1\%)\\
Compatibility among adjudicable covered tokens & 54/62 (87.1\%)\\
Underlying lexical-license errors identified & 1\\
\bottomrule
\end{tabular}
\end{table}

We call the final 54/62 quantity a \emph{compatibility rate among adjudicable
covered predicate tokens}, not accuracy. The denominator excludes predicates
for which grammar-v2 has no lexical license and the two cases whose linguistic
status remains unresolved. Moreover, token-level conflicts are not independent
grammar errors: all eight observed conflicts derive from the same frozen
lexical analysis of \textit{ro}. The holdout therefore supplies information
that the internal validation cannot: it exposes a reviewed generalization that
failed on held-out documentary material while preserving a transparent record
of the error rather than repairing the tested grammar.

\section{Limitations and scope of inference}
\label{sec:limits}

The v1 experiments are deliberately narrow. They contain five target lexemes and
...
architecture inside a reviewed domain and should not be interpreted as an
estimate of performance on unseen linguistic queries. Experiment~3 addresses
held-out documentary material separately, but its 33.0\% lexical coverage also
shows that grammar-v2 remains a partial grammar rather than a general model of
Bororo. It does not estimate the system's blocking rate for arbitrary requests. In particular, the absence of blocked proposals in
...
Experiment~1 shows that the frozen reviewed inventory supports recombination
beyond exact sentence retrieval: all 28 requested structural cells are realized
although none of the complete outputs is an exact normalized corpus sentence.
This result is modest but useful. It establishes that the baseline is not merely
returning stored sentences, while the provenance record still identifies
whether each morphological realization comes from an exact reviewed cell or a
reviewed full stem class. The experiment does not determine how far the same
architecture will generalize once the licensed inventory is expanded.

Experiment~2 addresses a different question and should not be read as a
head-to-head accuracy comparison between two autonomous generators. In
Condition~A, the model is responsible for the Bororo surface. In Condition~B,
the model is responsible only for preserving an abstract task specification;
the frozen deterministic grammar supplies the surface. The observed contrast
therefore localizes responsibility rather than establishing overall superiority
of one generation method.

That localization is visible in the results. The direct condition produces one
exact surface match to the controlled realization, while the linguistic audit
separates several reasons for nonmatch. A direct output may fail to instantiate
the requested indicative construction or requested participant value without
thereby being demonstrably morphologically ill formed. Other outputs remain
unresolved under the current analysis. Consequently, a binary exact-match score
would collapse distinctions that are relevant both linguistically and
computationally.

Condition~B, by contrast, tests whether the LLM can transmit the requested
abstract structure to the reviewed realization layer. It does so for all 28 v1
tasks: the submitted proposals preserve every audited task field, and the
frozen realizer returns a surface for each one. This result supports the
feasibility of the proposed division of labor for the licensed test set. It
does not establish that the model will preserve structure for unseen task
types, that the deterministic grammar covers Bororo generally, or that the
resulting surfaces constitute independent speaker judgments.

The main architectural consequence is therefore auditability. Corpus evidence,
human analysis, generation licenses, model proposals, deterministic
realizations, and post-generation attestation remain separately inspectable.
This separation makes it possible to identify whether uncertainty belongs to
the documentary evidence, the linguistic analysis, the requested structure, or
the realization layer. It also permits the system to preserve an unresolved
state instead of converting analogy or corpus frequency into an implicit
grammatical decision.

The experiment provides only indirect evidence about blocking. The
implementation fails closed when a required reviewed license is absent, but the
v1 LLM task set was sampled from licensed combinations and therefore contains
no blocking event. Demonstrating the behavior empirically will require a
separate challenge set containing systematically unsupported or partially
reviewed requests. Such a test would complement the present experiment rather
than change its interpretation.

The source-group holdout adds an independent diagnostic absent from the v1
tests. Among covered and adjudicable held-out predicates, 54/62 (87.1\%) are
compatible with the frozen grammar-v2. More importantly, the holdout identifies
one lexical generalization that does not survive evaluation: the frozen
divalent analysis of \textit{ro}. Its eight token-level conflicts are therefore
reported as repeated manifestations of one lexical-license error rather than as
eight independent grammatical rules. This illustrates why internal regression
tests and external generalization tests answer different questions.

Finally, versioning is essential to the human-review workflow. New corpus
observations and analyses discovered during audit can motivate later exact
cells, construction-specific realizations, or broader classes, but they must
enter a subsequent grammar version. Keeping those later decisions out of the
frozen v1 evaluation prevents experimental outputs from becoming retroactive
evidence for their own licensing.

\section{Conclusion}

This study operationalizes an evidential boundary between documentary
observation, human linguistic analysis, grammatical licensing, generation, and
post-generation attestation. In the Bororo case study, these states remain
separate computational objects: corpus occurrence can motivate analysis but
does not by itself create a productive license, and partial reviewed knowledge
can remain partial.

The frozen baseline realizes 28 licensed structural cells without retrieving an
exact complete sentence from the trusted corpus. In the LLM experiment, the
direct condition yields one exact surface match to the controlled realization
and requires a multidimensional linguistic audit of the remaining divergences.
In the evidence-bounded condition, all 28 abstract proposals preserve the
audited task specification and pass through the frozen deterministic realizer.
The latter result documents task preservation within the licensed v1 domain;
it is neither an independent grammaticality judgment nor evidence of general
Bororo coverage.

The methodological contribution is therefore not that corpus resources,
explicit grammar, human review, provenance, or LLM assistance are individually
new. It is the explicit representation and experimental use of
\emph{evidence-bounded licensing}: the model may propose a structure, but the
proposal does not acquire morphological or constructional authority merely by
being generated or corpus-plausible. This division of labor keeps uncertainty
and provenance inspectable and allows later linguistic analysis to extend a
versioned grammar without retroactively changing the evidence used to evaluate
earlier outputs.

The source-group holdout complements these internal tests: grammar-v2 covers
64 of 194 held-out predicate tokens, and 54 of 62 adjudicable covered tokens
(87.1\%) are compatible with its frozen licenses. The evaluation also exposes
one incorrect frozen lexical generalization, \textit{ro}: divalent, manifested
in eight held-out tokens. This distinction between internal functional
correctness, lexical coverage, held-out compatibility, unresolved evidence, and
underlying analytical errors is central to the evaluation protocol.

A further challenge set containing unsupported and partially reviewed requests
can evaluate abstention and blocking directly, but such a grammar-derived set
should be reported as an internal decision-boundary test rather than as
